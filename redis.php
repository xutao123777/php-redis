...
/*4.String，键值对，创建更新同操作*/
   ...
   $redis->setnx('key','value');//key=value,key存在返回false[|true], 将key的值设为value, 当且仅当key不存在。若给定的key已经存在, 则SETNX不做任何动作。SETNX是"SET if Not eXists"(如果不存在，则SET)的简写。
   $redis->setex('key','expire','value');  //将值value关联到key，并将key的生存时间设为seconds(以秒为单位)。如果key已经存在，SETEX命令将覆写旧值。在Redis用作缓存时，非常实用。
   ...
   注意: string型的数据结构, 值不能传递数组, 需要使用json_encode()或serialize()序列化数据。
...
/*5.List栈的结构,注意表头表尾,创建更新分开操作*/
   ...
   $redis->blpop('list', 60); //从开始处弹出一个值, 超时时间为60s   返回值: array ( 0 => 'list', 1 => '13697260356', )
   $redis->brpop('list', 60); //从结尾处弹出一个值, 超时时间为60s   返回值: array ( 0 => 'list', 1 => '18888888888', )

   b是blocking的意思, 也就是阻塞读。
   阻塞读在队列没有数据的时候, 会立即进入休眠状态, 一旦数据到来, 则立刻醒过来。消息的延迟几乎为零。用blpop/brpop替代前面的lpop/rpop, 就完美解决了上面的问题
   
   注意: list型的数据结构, 值不能传递数组, 需要使用json_encode()或serialize()序列化数据。
